...
//获取窗口信息
	var comboStore = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy({
			url : 'sys/windowAction!getWindowOfCenter.action?centerId='+ Ext.get("centerId").dom.value,
			method : 'POST'
		}),
	...
	combox2.on('select', function(combox){
		...
		rootNode.setText(combox.getRawValue() + "人员评价指标");
			   ...
			   if(itemStore.getCount()<=0){
				   ...
				   if(records.length > 0){
					   ...
					   Ext.Ajax.request({
			    			   ...
			    			   method : 'POST',
			    			   params : {"jsonData": data, itemId : firstItemId,userId :userId,sheetName:GLOBAL_MAP['SHEET_NAME'],sheetType :GLOBAL_MAP['SHEET_TYPE'], sheetId : GLOBAL_MAP['SHEET_ID']},
			...
			callback : function(options, success, response){
				if(success){
					...
					if(null == result || '' == result){
					   Ext.Msg.alert("提示信息","该窗口人员无人员可操作！");
					   return;
					}
...
//		alert(windowId + "|" + centerId + "|" + userId);
		...
		Ext.Ajax.request({
			...
			method : 'post',
			params : {'windowId' : windowId, 'centerId' : centerId,userId:userId},
			timeout : 10000,
			scope   : this,
			callback : function(options, success , response){
				if(success){
