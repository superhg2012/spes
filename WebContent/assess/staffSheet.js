			            	    	 ...
			            	    	 Ext.Ajax.request({
							    		...
							    		callback : function(success,responseText){
							    		   if(success){
							    			 ...
							    			 win.hide();
							    			 store.reload();
							    			// Ext.getCmp("doc-body").loadClass("assess/staffassess2.jsp", "绩效考核.窗口普通人员考核",true);
							    		   } else {
	...
	Ext.Ajax.request({
		...
		callback : function(opts, success, response){
			if(success){
				...
				Ext.getCmp("doc-body").loadClass("assess/staffassess.jsp", "绩效考核.窗口人员考核" ,true);
			} else {
	...
	Ext.Ajax.request({
		...
		callback : function(opts, success, response){
			if(success){
		    	...
		    	GLOBAL_MAP['CHECK_TYPE'] = json.checkType;//staff
		    	GLOBAL_MAP['SHEET_RAWTYPE'] = ExtJSUtil.getRawSheetType(json.sheetType);
				Ext.getCmp("doc-body").loadClass("assess/staffassess2.jsp", "绩效考核.窗口人员考核->续" ,true);
			} else {
				alert("errors encounter when requesting info from database!");
				return;
			}
