Ext.onReady(function(){
	    ...
	    var store = new Ext.data.Store({
		 proxy : new Ext.data.HttpProxy({
			  url : 'center/evaluateCSheetAction!getHisEvaluateSheets.action?checkType=window',
			  method  : 'POST'
		 }), 
			   ...
			   text: '创建窗口考核表单',
	            ...
	            handler: function(obj){
			      ...
			      if(!win){
			            ...
			            modal : true,
			            labelWidth:60,
			            resizable:false, //resizable：是否可以改变窗口大小
			            minimizable:true,
			            closeAction:'hide',
			            constrain:false,
			            plain: true, //plain：渲染窗口的背景颜色
			                    ...
			                    handler :function(buttonObj){
			            	      ...
			            	      if( sheetName!="" && checktype!=""){
			            	    	 Ext.Ajax.request({
							    		...
							    		method : 'post',
							    		params : {'sheetName' : sheetName,'sheetType': checktype,'checkType' : 'window'},
							    		callback : function(success,responseText){
							    		   if(success){
							    			  ...
							    			  win.hide();
							    			  store.reload();
										   //   Ext.getCmp("doc-body").loadClass("assess/windowassess.jsp", "绩效考核.窗口考核",true);
							    		   } else {
							    			   Ext.MessageBox.alert("错误提示","创建考核表单失败！请重新尝试！");
							    			   return;
							    		   }
							    		}
							    	});
	...
	Ext.Ajax.request({
		...
		callback : function(opts, success, response){
			if(success){
		    	...
		    	GLOBAL_MAP['CHECK_TYPE'] = json.checkType;
	...
	Ext.Ajax.request({
		...
		callback : function(opts, success, response){
			if(success){
		    	...
		    	GLOBAL_MAP['SHEET_NAME']= json.sheetName;
		    	GLOBAL_MAP['SHEET_TYPE'] = json.sheetType;
		    	GLOBAL_MAP['CKECK_TYPE'] = json.ckeckType;
		    	GLOBAL_MAP['SHEET_RAWTYPE'] = ExtJSUtil.getRawSheetType(json.sheetType);
				Ext.getCmp("doc-body").loadClass("assess/windowassess2.jsp", "绩效考核.窗口考核->续" ,true);
			} else {
				alert("errors encounter when requesting info from database!");
				return;
			}
